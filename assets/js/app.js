...
(function () {
  ...
  function phaseBadge(label, phase) {
    var span = el("span", "gchip gchip--phase");
    if (phase) span.setAttribute("data-ko", phase);
    span.title = label;
  ...
  function matchRow(m, opts) {
    ...
    left.appendChild(m.phase === "girone" ? gchip(m.group, { letterOnly: true }) : phaseBadge(m.phaseLabel, m.phase));
